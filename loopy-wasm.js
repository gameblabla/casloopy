...
const DEFAULT_CONFIG = { keys:{...DEFAULT_KEYS}, video:{ aspect:'native', smoothUpscale:false, scanlines:false, showFps:true }, stateSlot:0, portDevice:'gamepad' };
const els = {
  ...
  biosFile: document.getElementById('biosFile'), soundFile: document.getElementById('soundFile'), biosStatus: document.getElementById('biosStatus'), soundStatus: document.getElementById('soundStatus'), biosCard: document.getElementById('biosCard'), soundCard: document.getElementById('soundCard'),
...
function loadConfig() {
  try {
    ...
    if (raw) {
      ...
      return { ...DEFAULT_CONFIG, ...saved, keys:{...DEFAULT_KEYS, ...(saved.keys || {})}, video:{...DEFAULT_CONFIG.video, ...(saved.video || {})}, portDevice: saved.portDevice === 'mouse' ? 'mouse' : 'gamepad', stateSlot: Number.isInteger(saved.stateSlot) ? saved.stateSlot : 0 };
    }
  } catch (_) {}
...
function romTone(crc, expected){ return (crc>>>0) === (expected>>>0) ? 'ok' : 'warn'; }
function romStatus(entry, expected, label){ if (!entry) return 'missing'; const ok = (entry.crc>>>0) === (expected>>>0); return `${entry.name} (${entry.size.toLocaleString()} bytes, ${fmtCrc(entry.crc)}${ok ? '' : `; expected ${fmtCrc(expected)}`})${ok ? '' : ' — warning: ROM may be invalid'}`; }
function updateRomStatus(){ const bios=manifest.roms?.bios, sound=manifest.roms?.sound; els.biosStatus.textContent = romStatus(bios, EXPECTED_BIOS_CRC, 'BIOS'); els.soundStatus.textContent = romStatus(sound, EXPECTED_SOUND_CRC, 'Sound'); els.biosStatus.className = bios ? (romTone(bios.crc, EXPECTED_BIOS_CRC)==='ok'?'status-ok':'status-warn') : 'status-bad'; els.soundStatus.className = sound ? (romTone(sound.crc, EXPECTED_SOUND_CRC)==='ok'?'status-ok':'status-warn') : 'status-bad'; els.biosCard.classList.toggle('ready', !!bios && bios.crc === EXPECTED_BIOS_CRC); els.soundCard.classList.toggle('ready', !!sound && sound.crc === EXPECTED_SOUND_CRC); els.biosCard.classList.toggle('warn', !!bios && bios.crc !== EXPECTED_BIOS_CRC); els.soundCard.classList.toggle('warn', !!sound && sound.crc !== EXPECTED_SOUND_CRC); }
async function storeRom(kind, file){ if (!file) return; const bytes = await fileToBytes(file); const crc = crc32(bytes); const expected = kind === 'bios' ? EXPECTED_BIOS_CRC : EXPECTED_SOUND_CRC; const key = `rom:${kind}`; await dbPut(key, bytes); manifest.roms[kind] = { key, name:file.name, size:file.size, crc, storedAt:new Date().toISOString() }; saveManifest(); updateRomStatus(); updateStorageStatus(`${kind === 'bios' ? 'internal ROM' : 'sound ROM'} stored: ${fmtCrc(crc)}${crc === expected ? '' : ' (warning: CRC mismatch)'}`, crc === expected ? 'ok' : 'warn'); }
async function loadStoredRom(kind){ const entry = manifest.roms?.[kind]; if (!entry) throw new Error(`${kind === 'bios' ? 'Internal ROM' : 'Sound ROM'} is missing`); const raw = await dbGet(entry.key); if (!raw) throw new Error(`${entry.name} metadata exists but IndexedDB payload is missing`); return raw instanceof Uint8Array ? raw : new Uint8Array(raw); }
...
async function startOrRestart(){ if (!activeCartBytes) throw new Error('Load a cartridge first'); unlockAudioFromGesture(); wasm.loopy_wasm_reset_heap(); const bios = await loadStoredRom('bios'); const sound = await loadStoredRom('sound'); let ptr = copyBytesToWasm(bios); const biosCrc = wasm.loopy_wasm_load_bios(ptr, bios.byteLength) >>> 0; ptr = copyBytesToWasm(sound); const soundCrc = wasm.loopy_wasm_load_sound_rom(ptr, sound.byteLength) >>> 0; ptr = copyBytesToWasm(activeCartBytes); wasm.loopy_wasm_load_cart(ptr, activeCartBytes.byteLength); const sram = await dbGet(sramKey()); if (sram) { const u8 = sram instanceof Uint8Array ? sram : new Uint8Array(sram); ptr = copyBytesToWasm(u8); wasm.loopy_wasm_load_sram(ptr, u8.byteLength); }
  const ok = wasm.loopy_wasm_start(config.portDevice === 'mouse' ? 1 : 0); if (!ok) throw new Error(`backend rejected startup, error 0x${(wasm.loopy_wasm_get_error()>>>0).toString(16)}`); autoPausedForHiddenTab = false; paused = false; running = true; applyPortDevice({status:false}); clearAudioQueue(); resetFrameLimiter(); els.pauseToggle.textContent = 'Pause'; els.startupModal.classList.remove('open'); updateStorageStatus(`running; BIOS ${fmtCrc(biosCrc)}, sound ${fmtCrc(soundCrc)}, input ${actualPortDeviceName()}`, (biosCrc===EXPECTED_BIOS_CRC && soundCrc===EXPECTED_SOUND_CRC) ? 'ok' : 'warn'); }
...
function buttonsMask(){ let mask = 0; for (const [name, bit] of BUTTONS) if (pressed.has(config.keys[name])) mask |= bit; return mask >>> 0; }
...
function applyConfigToControls(){ els.aspectMode.value = config.video.aspect; els.smoothUpscale.checked = !!config.video.smoothUpscale; els.scanlines.checked = !!config.video.scanlines; els.showFps.checked = config.video.showFps !== false; els.stateSlot.value = String(config.stateSlot); els.portDevice.value = config.portDevice; renderOnce(); }
...
function applyPortDevice(opts={}){ const wantMouse = config.portDevice === 'mouse'; wasm?.loopy_wasm_set_port_device?.(wantMouse ? 1 : 0); resetMouseTracking(); pressed.clear(); if(!wantMouse) releaseMouseCapture(); syncMouseCaptureUI(); if(opts.status) updateStorageStatus(`input device set to ${actualPortDeviceName()}`, 'ok'); }
function applyRuntimeOptions(){ config.video.aspect = els.aspectMode.value; config.video.smoothUpscale = els.smoothUpscale.checked; config.video.scanlines = els.scanlines.checked; config.video.showFps = els.showFps.checked; config.stateSlot = parseInt(els.stateSlot.value,10)||0; const selectedDevice = els.portDevice.value === 'mouse' ? 'mouse' : 'gamepad'; const oldDevice = config.portDevice; config.portDevice = selectedDevice; saveConfig(); applyPortDevice({status: oldDevice !== selectedDevice}); renderOnce(); }
...
function wireEvents(){
  ...
  els.openStartup.addEventListener('click',()=>els.startupModal.classList.add('open')); els.closeStartup?.addEventListener('click', closeRomManager); els.closeStartupAlt?.addEventListener('click', closeRomManager); els.menuToggle.addEventListener('click',()=>els.sideMenu.classList.toggle('open')); els.closeMenu.addEventListener('click',()=>els.sideMenu.classList.remove('open'));
  els.fullscreenToggle.addEventListener('click',()=>setFullscreen(!document.fullscreenElement)); document.addEventListener('fullscreenchange', updateFullscreenUI); window.addEventListener('resize',()=>requestAnimationFrame(()=>renderOnce()));
  els.biosFile.addEventListener('change',()=>storeRom('bios', els.biosFile.files[0]).catch(e=>updateStorageStatus(e.message,'bad'))); els.soundFile.addEventListener('change',()=>storeRom('sound', els.soundFile.files[0]).catch(e=>updateStorageStatus(e.message,'bad')));
  ...
  for (const el of [els.aspectMode, els.smoothUpscale, els.scanlines, els.showFps, els.stateSlot, els.portDevice]) el.addEventListener('change', applyRuntimeOptions);
